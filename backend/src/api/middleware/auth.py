...
async def require_music_school_or_admin(current_user: User = Depends(get_current_user)) -> User:
    from src.domain.user.value_objects import UserRole
    if current_user.role not in (UserRole.ADMIN, UserRole.MUSIC_SCHOOL):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Musiqa maktabi yoki Admin huquqi talab qilinadi",
        )
    return current_user
