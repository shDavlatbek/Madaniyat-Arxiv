...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar :title="`${year} yil - Kategoriyalar`">
        <template #left>
    ...
    <template #body>
      ...
      <div v-else class="flex items-center justify-center p-12">
        <UEmpty icon="i-lucide-folder-x" title="Kategoriyalar topilmadi" :description="`${year} yil uchun kategoriyalar mavjud emas`" />
      </div>
    </template>
  </UDashboardPanel>
</template>
